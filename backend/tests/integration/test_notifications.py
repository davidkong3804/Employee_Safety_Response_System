...
class TestTriggerReminders:
    async def test_manager_reminds_own_department_unreported(self, client, manager_headers, active_event):
        ...
        r = await client.post(f"/api/events/{active_event.id}/remind", headers=manager_headers)
        assert r.status_code == 200
        data = r.json()
        assert data["reminded_count"] == 2
        assert "2" in data["message"]

    async def test_excludes_already_reported_users(self, client, manager_headers, employee_headers, active_event):
        ...
        r = await client.post(f"/api/events/{event_id}/remind", headers=manager_headers)
        # Engineering scope minus already-reported employee → only manager left.
        assert r.json()["reminded_count"] == 1

    async def test_reminder_count_increments_on_repeat(self, client, manager_headers, active_event):
        event_id = str(active_event.id)
    ...
    async def test_admin_reminds_across_all_departments(self, client, admin_headers, active_event):
        # Admin sees the whole event — IT (admin) + Engineering (manager,
        # employee) → 3. Confirms admin is not narrowed by the manager
        # dept-scope rule.
        r = await client.post(f"/api/events/{active_event.id}/remind", headers=admin_headers)
        assert r.status_code == 200
        assert r.json()["reminded_count"] == 3

    async def test_employee_cannot_trigger_reminders(self, client, employee_headers, active_event):
        r = await client.post(f"/api/events/{active_event.id}/remind", headers=employee_headers)
        assert r.status_code == 403

    async def test_excludes_inactive_users(self, client, manager_headers, active_event, db_session):
...
class TestGetReminders:
    async def test_empty_before_any_reminder_triggered(self, client, manager_headers, active_event):
        r = await client.get(f"/api/events/{active_event.id}/reminders", headers=manager_headers)
        assert r.status_code == 200
        assert r.json() == []

    async def test_returns_reminders_after_trigger(self, client, manager_headers, active_event):
        event_id = str(active_event.id)
        await client.post(f"/api/events/{event_id}/remind", headers=manager_headers)
    ...
    async def test_employee_cannot_get_reminders(self, client, employee_headers, active_event):
        r = await client.get(f"/api/events/{active_event.id}/reminders", headers=employee_headers)
        assert r.status_code == 403
